...
async function loadStartBuffer() {
    const saved = await browser.storage.local.get('startBuffer').catch(() => ({}));
...
async function stopSpeech(killModel = true) {
    stopped = true;
    queue = [];
    clearPlayer();
    setActive(false);
...
browser.runtime.onMessage.addListener((request) => {
    ...
    if (request.action === 'showGeneratingSpeech') setActive(true);
    else if (request.action === 'queueTTSAudio') {
        return dataUrlToBlob(request.audioUrl).then(blob => {
            if (request.ownerToken !== ownerToken || stopped) return;
    ...
    } else if (request.action === 'ttsProducerComplete') { producerComplete = true; if (!playing) playNext(); }
    else if (request.action === 'stopTTSAudio') { stopped = true; queue = []; clearPlayer(); setActive(false); }
    else if (request.action === 'ttsError') { stopped = true; setActive(false); showStatus(request.error || 'Chatterbox generation failed', 'error'); }
    return Promise.resolve({ success: true });
});

async function activeTab() { return (await browser.tabs.query({ active: true, currentWindow: true }))[0]; }
async function getSelectedText() {
    try {
        const tab = await activeTab();
        const result = await browser.tabs.executeScript(tab.id, { code: 'window.getSelection().toString().trim()' });
        textInput.value = (result && result[0]) || '';
...
async function getPageText() {
    try {
        const tab = await activeTab();
        const result = await browser.tabs.executeScript(tab.id, { code: `(function(){const w=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT,{acceptNode(n){const p=n.parentElement;return p&&['SCRIPT','STYLE','NOSCRIPT'].includes(p.tagName)?NodeFilter.FILTER_REJECT:NodeFilter.FILTER_ACCEPT}});let out='',n;while(n=w.nextNode()){const t=n.textContent.trim();if(t)out+=t+'\n'}return out.trim().substring(0,5000)})()` });
        textInput.value = (result && result[0]) || '';
...
async function openVoiceLab() {
    ...
    catch (_) { await browser.runtime.openOptionsPage(); }
}
