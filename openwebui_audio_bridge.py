...
CHATTERBOX_ACCELERATOR_BASE = os.environ.get('CHATTERBOX_ACCELERATOR_BASE', 'http://127.0.0.1:8021')
CHATTERBOX_ACCELERATOR_SERVICE = os.environ.get('CHATTERBOX_ACCELERATOR_SERVICE', 'chatterbox-nano-accelerator.service')
CHATTERBOX_ACCELERATOR_ENABLED = os.environ.get('CHATTERBOX_ACCELERATOR_ENABLED', '').strip().lower() in {'1', 'true', 'yes', 'on'}
CHATTERBOX_ACCELERATOR_LABEL = os.environ.get('CHATTERBOX_ACCELERATOR_LABEL', 'GPU / accelerator').strip() or 'GPU / accelerator'
...
def check_auth():
    ...
    if auth.split(' ', 1)[1].strip() != BRIDGE_API_KEY:
        return False, jsonify({'error': 'invalid api key'}), 401
    return True, None, 200


def requested_backend(device: object) -> tuple[str, str, str]:
    choice = str(device or '').strip().lower()
    # No hint is intentionally CPU so existing OpenAI/Open WebUI clients keep
    # their previous behaviour. The browser sends "auto" explicitly.
    if choice in {'gpu', 'accelerator', 'cuda'}:
        if not CHATTERBOX_ACCELERATOR_ENABLED:
            raise ValueError('No GPU/accelerator backend is configured on this installation')
        return CHATTERBOX_ACCELERATOR_BASE, CHATTERBOX_ACCELERATOR_SERVICE, 'gpu'
    if choice == 'auto' and CHATTERBOX_ACCELERATOR_ENABLED:
        return CHATTERBOX_ACCELERATOR_BASE, CHATTERBOX_ACCELERATOR_SERVICE, 'gpu'
    return CHATTERBOX_BASE, CHATTERBOX_SERVICE, 'cpu'


def wait_for_chatterbox(base: str, timeout_s: float = STARTUP_WAIT_SECONDS) -> bool:
    deadline = time.time() + timeout_s
    while time.time() < deadline:
        try:
            if requests.get(f'{base}/health', timeout=1.5).ok:
                return True
        except Exception:
            pass
...
def ensure_chatterbox_running(base: str, service: str) -> bool:
    with _chatterbox_start_lock:
        if wait_for_chatterbox(base, 1.0):
            return True
        subprocess.run(['systemctl', '--user', 'start', service], check=False, capture_output=True, text=True)
        return wait_for_chatterbox(base)
...
def get_whisper_worker() -> subprocess.Popen[str]:
    ...
    if not whisper_configured():
        raise RuntimeError('Whisper STT is not configured; set WHISPER_MODEL and WHISPER_WORKER')
    ...
    env.setdefault('WHISPER_CPU_THREADS', str(max(1, (os.cpu_count() or 2) - 1)))
    _whisper_worker = subprocess.Popen(
        [WHISPER_PYTHON, str(Path(WHISPER_WORKER).expanduser())],
...
def backend_health(base: str) -> dict:
    try:
        response = requests.get(f'{base}/health', timeout=1.0)
        if response.ok:
            detail = response.json()
            return {'running': True, 'device': detail.get('device'), 'label': detail.get('device_label')}
    except Exception:
        pass
...
@app.get('/health')
def health():
    cpu = backend_health(CHATTERBOX_BASE)
    accelerator = backend_health(CHATTERBOX_ACCELERATOR_BASE) if CHATTERBOX_ACCELERATOR_ENABLED else {'running': False}
    devices = [
        {'id': 'auto', 'label': 'Auto', 'available': True},
        {'id': 'cpu', 'label': 'CPU', 'available': True, **cpu},
        {
            'id': 'gpu',
            'label': accelerator.get('label') or CHATTERBOX_ACCELERATOR_LABEL,
            'available': CHATTERBOX_ACCELERATOR_ENABLED,
            **accelerator,
        },
    ]
    return jsonify({
        ...
        'synthesis_devices': devices,
        'accelerator_enabled': CHATTERBOX_ACCELERATOR_ENABLED,
        'default_stt_backend': 'whisper' if whisper_configured() else None,
...
def tts_speech():
    ...
    try:
        base, service, resolved_device = requested_backend(body.get('device') or request.headers.get('X-Chatterbox-Device'))
    except ValueError as exc:
        return jsonify({'error': str(exc)}), 400
    if not ensure_chatterbox_running(base, service):
        return jsonify({'error': f'{resolved_device} chatterbox backend did not become ready'}), 502
    ...
    for raw_chunk in split_tts_text(text):
        ...
        try:
            ...
            if not ensure_chatterbox_running(base, service):
                return jsonify({'error': 'chatterbox request failed and backend is not healthy'}), 502
            try:
                response = requests.post(
                    f'{base}/v1/audio/speech',
                    json=payload,
    ...
    result = Response(audio, mimetype='audio/wav')
    result.headers['X-Chatterbox-Device'] = resolved_device
    return result
...
def stt_transcribe():
    ...
    if not whisper_configured():
        return jsonify({'error': 'Whisper STT is not configured on this installation'}), 501
    with tempfile.TemporaryDirectory(prefix='whisper_stt_') as td:
        src = Path(td) / (f.filename or 'input_audio')
    ...
    return jsonify({'text': text, 'backend': 'whisper', 'duration_seconds': round(time.monotonic() - started, 3)})
